...
export function registerCommandTools(server: McpServer) {
  ...
  server.tool(
    ...
    {
      deviceId: z.number().describe('Device ID to load mission'),
      ...
      if (!plan || !plan.missionData) {
        return {
          content: [
            {
              type: 'text',
              text: `Mission plan id ${missionPlanId}  , please check the plan ID and try again.`,
            },
          ],
          isError: true,
        };
  ...
  server.tool(
    ...
    'Send a high level task to GCS for UAV inspection',
    {
      ...
      objetivo: z.number().describe('Objective: 0=Location, 1=Bird inspection, 2=Lidar survey'),
      loc: z
        .array(
          z.object({
